...
def auchan(IDENTIFIANT_AUCHAN, PASSWORD_AUCHAN, WEB_BROWSER):
    ...
    id_produits = csv_reader(file='Course.csv', row_number=6)
    tva_produits = csv_reader(file='Course.csv', row_number=7)
    ...
    prix = [('Produit', 'Nombre de lots à acheter', 'Nombre de produits par lots', 'Prix', 'REF', 'ID du produit', 'TVA')]
    ...
    def ajout_produit(row):
        ...
        try:
            ...
            return
        prix.append((produits[row], nb_de_lots_a_acheter[row], nb_produits_par_lots[row], prix_produit, ref_produits_auchan[row], id_produits[row], tva_produits[row]))
        for i in range(int(nb_de_lots_a_acheter[row])):
            try:
                ...
                if i == 0:
                    ...
                    prix[-1] = (produits[row], i, nb_produits_par_lots[row], prix_produit, ref_produits_auchan[row], id_produits[row], tva_produits[row])
